...
sender - who is sending or transfering

receiver - who is receiving

amount - how much he is receiving

timestamp - when this transaction took place

status - what is the status of the transaction (e.g. PENDING=0, CONFIRMED=1)

id - a unique key to identify each transaction respectively

signatureBase64 = signature of the canonical string (e.g. sender|receiver|amount|timestamp = ot/hHmiOtUxFjL6D3xJ6NTw8CHuSLwDfP0Dbnc10mQUv3cwfvwx4qk7Wyebk/sn8DOPK9UprkjieJ0NkSvOE/Q==)

pubKeyPem - sender's public key in PEM format (e.g. "-----BEGIN PUBLIC KEY-----\nMFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEgdLB/VWXLvUSmv/Dkp5S7yH3rFko\nwxvvClZ3IIRANqc8UDUSHFT4ovlJombbusqXBcZETgAP9OiVQkUJ1jhYUA==\n-----END PUBLIC KEY-----")


Helper functions for Transaction Class
...
timestamp - when was the block formed or created

transactions - it is a vector that stores some transactions for that block, these transactions will be later verified when the block will be mined by the miner.

previousHash - the previous block's hash. it is necessary as each block depends on its previous block. as in blockchain each block is connected with its previous block so it ensures security as nobody can inject another unverified or corrupt block at any moment. so, its just the previous block's hash

hash - a cryptographic hash for this block and this will act as previous hash for the next block of the blockchain.

nonce - it is a integer value and nonce means number used once. a nonce in a block class is used to store the count at which a ideal hash is found for the block.


Helper functions for the Block class

calculateHash() - returns a cryptographic hash string that is generated using a canonical string
(e.g. index|timestamp|previousHash|nonce)
...
chain - a vector that stores blocks with confirmed status transactions. blocks inside the chain vector means those blocks are already mined and now inside the real blockchain.

mempool - a vector that stores transactions with pending status. transactions inside the mempool are not yet put inside a block, they are there for the miners. the miners will come take these transaction create a block with them and then mine the created block and then add the verified block to the chain vector.
...
userToWallet - it is an unordered map and this is used to store users wallet address against their userId, in our case, we get the userId from the frontend as clerk's userId

walletBalances - it is also an unordered list that stores users balances against their wallet address or wallet id (both are same).

walletPublicKey - it is an unordered list that stores the publicKeyPEM of against their wallet address. storing public id is necessary as this is used for verifying a transaction beforing adding to the mempool vector and authorize an user.
